from database import save_prediction_history
...
def show_model():
    ...
    if st.button("🧠 Predict Chronic Kidney Disease"):
        ...
        prediction_proba_all = rf_model.predict_proba(model_input)[0]

        prediction_proba = prediction_proba_all[prediction]
        egfr_value = calculate_egfr_ckd_epi_2021(
            serum_creatinine,
            age,
            gender
        )
    ...
    # ==========================================
    # Show Prediction + eGFR + Interpretation
    # ==========================================
    ...
    if st.session_state.prediction_done:
        ...
        if prediction == 1:
            ...
            st.markdown(
                "There is **Chronic Kidney Disease** based on the entered parameters. "
                "Further evaluation and laboratory tests are recommended."
            )
            ...
            st.markdown(
                "There is **No Chronic Kidney Disease** based on the entered parameters. "
                "However, regular monitoring is advised."
            )

        st.markdown(
            f"**Model Confidence Score:** {prediction_proba:.2f}"
        )

        if prediction_proba < 0.7:
            st.warning(
                "⚠️ The model's confidence is below 70%."
            )
        ...
        st.caption(
            f"**eGFR**: {egfr_value} mL/min/1.73m²"
        )
        ...
        if st.session_state.history_saved:
             st.success("✅ Prediction saved successfully!")
        if st.button("📊 Show Explainable AI Analysis"):
    ...
    if st.session_state.show_shap:
        ...
        prediction_prob = pred_proba[pred_class]  # From pred_proba = rf_model.predict_proba(input_df)[0]
        ...
        predicted_class = rf_model.predict(model_input)[0]

        # Get the probability of the predicted class
        predicti_prob = rf_model.predict_proba(model_input)[0][predicted_class]
        ...
        pred_proba = rf_model.predict_proba(input_df)[0]
        pred_class = rf_model.predict(input_df)[0]  # Predicted class
        class_names = {0: "Negative", 1: "Positive"}
